import {Switch} from "@/components/ui/switch";
import {Label} from "@/components/ui/label";
import {Select, SelectTrigger, SelectValue, SelectContent, SelectItem} from "@/components/ui/select";
import {useState} from "react";

const AppPreferencesSetting = () => {
    const [preferences, setPreferences] = useState({
        language: "english",
        region: "us",
        darkMode: false
    });

    const handleSelectChange = (key: "language" | "region", value: string) => {
        setPreferences((prev) => ({...prev, [key]: value}));
    };

    return (
        <article className="mt-6">
            <header className="font-bold text-2xl">App Preferences</header>
            <section className="space-y-4 mt-4 *:px-1">
                <div className="flex justify-between items-center hover:bg-gray-300 rounded py-1">
                    <Label htmlFor="language" className={'w-full'}>Language</Label>
                    <Select defaultValue={preferences.language}
                            onValueChange={(value) => handleSelectChange("language", value)}>
                        <SelectTrigger className="w-40">
                            <SelectValue placeholder="Select Language"/>
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value={'vietnamese'}>Vietnamese</SelectItem>
                            <SelectItem value="english">English</SelectItem>
                            <SelectItem value="spanish">Spanish</SelectItem>
                            <SelectItem value="french">French</SelectItem>
                            <SelectItem value="german">German</SelectItem>
                            <SelectItem value="japanese">Japanese</SelectItem>
                        </SelectContent>
                    </Select>
                </div>

                <div className="flex justify-between items-center hover:bg-gray-300 rounded py-1">
                    <Label htmlFor="region" className={'w-full'}>Region</Label>
                    <Select defaultValue={preferences.region}
                            onValueChange={(value) => handleSelectChange("region", value)}>
                        <SelectTrigger className="w-40">
                            <SelectValue placeholder="Select Region"/>
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value={'vietnamese'}>Vietnamese</SelectItem>
                            <SelectItem value="us">United States</SelectItem>
                            <SelectItem value="eu">Europe</SelectItem>
                            <SelectItem value="asia">Asia</SelectItem>
                            <SelectItem value="australia">Australia</SelectItem>
                        </SelectContent>
                    </Select>
                </div>

                <div className="flex justify-between items-center hover:bg-gray-300 rounded py-2">
                    <Label htmlFor="darkMode" className="text-lg w-full">
                        <div>
                            <h1>Dark Mode</h1>
                            <p className="text-gray-500 font-light text-sm">Enable dark theme for better night
                                experience.</p>
                        </div>
                    </Label>
                    <Switch id="darkMode" checked={preferences.darkMode}
                            onCheckedChange={(value) => setPreferences((prev) => ({...prev, darkMode: value}))}/>
                </div>
            </section>
        </article>
    );
};

export default AppPreferencesSetting;
